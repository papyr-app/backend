...
def create_virtual_path_bp():
    ...
    def create_virtual_path(user: User):
        ...
        try:
            ...
            virtual_path = virtual_path_service.create_virtual_path(user, **validated_data)
    ...
    def update_virtual_path(user: User, virtual_path_id: str):
        ...
        try:
            ...
            virtual_path = virtual_path_service.update_virtual_path(virtual_path, validated_data)
            return jsonify({'data': virtual_path.to_mongo().to_dict()}), 200
        except ValidationError as err:
            return jsonify({"error": str(err)}), 400
        except DoesNotExist:
